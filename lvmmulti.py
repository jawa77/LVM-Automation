...
def copy_data(unique_name):
    ...
    run_command(f"sudo rsync -aAXvP --owner --group {source_path}/* {mount_point}")
...
def main():
    ...
    if mode == "multi":
        # Use multiprocessing to process each LV creation in parallel
        num_processes = min(len(unique_names), cpu_count())
        print(num_processes)
        with Pool(num_processes) as pool:
            pool.starmap(process_unique_name, [(name.strip(), storage_size) for name in unique_names])
    else:
        # Process LV creation sequentially
